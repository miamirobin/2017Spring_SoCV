...
genProofModel(SatSolver& s)
{
   ...
   s.addAigCNF(gates[8]->getVar(), gates[7]->getVar(), true,
               gates[7]->getVar(), true);
    s.addAigCNF(gates[8]->getVar(), gates[6]->getVar(), true,
               gates[6]->getVar(), true);
    s.addAigCNF(gates[8]->getVar(), gates[0]->getVar(), true,
                gates[0]->getVar(), true);

    s.addAigCNF(gates[9]->getVar(), gates[8]->getVar(), true,
                gates[0]->getVar(), true);
...
int main()
{
   ...
   reportResult(solver, result);
    ...
    solver.addAigCNF(newV1, gates[0]->getVar(), false, gates[1]->getVar(), false);
    solver.assumeRelease();  // Clear assumptions
    solver.assumeProperty(newV1, true);
    result = solver.assumpSolve();
    reportResult(solver, result);
    solver.assumeRelease();  // Clear assumptions
    solver.assumeProperty(gates[9]->getVar(), true);
    result = solver.assumpSolve();
    reportResult(solver, result);
}
